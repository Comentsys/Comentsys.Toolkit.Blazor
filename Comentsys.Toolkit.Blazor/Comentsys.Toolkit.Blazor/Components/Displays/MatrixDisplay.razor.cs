...
/// <summary>
/// Matrix Display Component for Blazor WebAssembly or Blazor Server with IsRealTimeForWebAssembly as False
/// </summary>
public partial class MatrixDisplay
{
    ...
    private const decimal segment_height = 120.25m;
    ...
    private const string svg_start = "<svg viewBox=\"0 0 86.5 120.25\" fill=\"none\" xmlns=\"http://www.w3.org/2000/svg\">";

    private readonly List<(Value value, Style style, string group, string fill)> _glyphs = [];
    ...
    private void CacheGlyphs(Style style, Color fill)
    {
        var htmlFill = fill.AsHtmlColor() ?? black;
        foreach (Value value in Enum.GetValues<Value>())
        {
            if (_glyphs.Any(g => g.style == style && g.value == value && g.fill == htmlFill))
                continue;
            var svg = Matrix.Get(value, fill, style)?.ToSvgString();
            if (string.IsNullOrWhiteSpace(svg))
                continue;
            var glyph = ExtractGlyph(svg);
            _glyphs.RemoveAll(g => g.value == value && g.fill == htmlFill);
            _glyphs.Add((value, style, glyph ?? string.Empty, htmlFill));
        }
    }

    /// <summary>
    /// Get Glyph SVG by Style, Value and Fill
    /// </summary>
    /// <param name="style">Style</param>
    /// <param name="value">Value</param>
    /// <param name="fill">Fill</param>
    /// <returns>Glyph</returns>
    private string GetGlyph(Style style, Value value, string fill)
    {
        var glyph = _glyphs.FirstOrDefault(g => g.style == style && g.value == value && g.fill == fill);
    ...
    private void Update()
    {
        ...
        if (!values.SequenceEqual(_values))
        {
            ...
            foreach (var (v, i) in values.Select((v, i) => (v, i)))
            {
                ...
                var glyph = GetGlyph(style, v, fill);
    ...
    /// </summary>
    [Parameter]
    public Style? Style { get; set; } = Comentsys.Assets.Display.Style.Square;
